...
I2C_HandleTypeDef 	*bh1750_i2c;	// Handler do i2c
bh1750_mode 		Bh1750_Mode;	// sensor mode
uint8_t 			Bh1750_Mtreg;	// wartosc rejestru
...
BH1750_STATUS BH1750_SetMtreg(uint8_t Mtreg)
{
	...
	retCode = HAL_I2C_Master_Transmit(bh1750_i2c, BH1750_ADDRESS, &tmp[0], 1, 1000);
	if (HAL_OK != retCode) {
		return BH1750_ERROR;
	}

	retCode = HAL_I2C_Master_Transmit(bh1750_i2c, BH1750_ADDRESS, &tmp[1], 1, 1000);
	if (HAL_OK == retCode) {
		return BH1750_OK;
	}
...
BH1750_STATUS BH1750_ReadLight(float *Result)
{
	...
	if(HAL_OK == HAL_I2C_Master_Receive(bh1750_i2c, BH1750_ADDRESS, tmp, 2, 1000))
	{
